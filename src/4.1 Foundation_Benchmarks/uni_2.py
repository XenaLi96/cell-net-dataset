...
# =============================
# Define UNI-2 as Feature Extractor
# =============================
class UNI2Encoder(nn.Module):
    def __init__(self, model_path):
        super(UNI2Encoder, self).__init__()
        timm_kwargs = {
        ...
        self.encoder = timm.create_model(pretrained=False, **timm_kwargs)
        self.encoder.load_state_dict(torch.load(model_path, map_location="cpu"), strict=True)
...
model = UNI2Encoder("/home/yaqih10/CellNet/self_benchmarks/uni_2.bin").to(device)
model.eval()
...
# Save Results
pd.DataFrame(results).T.to_csv("uni_2_result.csv")
