...
# =============================
# Define H-Optimus-0 as Feature Extractor
# =============================
class HOptimus0Encoder(nn.Module):
    def __init__(self, weights_path="/home/yaqih10/CellNet/self_benchmarks/hoptimus0.bin"):
        super(HOptimus0Encoder, self).__init__()
        ...
        # Load custom weights from the local path
        state_dict = torch.load(weights_path, map_location="cpu")
        self.encoder.load_state_dict(state_dict)

    def forward(self, x):
        # Forward pass through H-Optimus-0 encoder
        return self.encoder(x)  # Output shape: (batch_size, 1536)
...
# Save Results
pd.DataFrame(results).T.to_csv("hoptimus_results.csv")
