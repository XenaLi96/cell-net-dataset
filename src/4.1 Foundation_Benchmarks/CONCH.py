...
# =============================
# Feature Extraction using CONCH
# =============================
    ...
    for images, gene_exprs, patch_ids_batch in tqdm(dataloader, desc="Extracting Features"):
        ...
        for img in images:
            if isinstance(img, torch.Tensor):
                img = transforms.ToPILImage()(img)  # Convert tensor to PIL if needed
            img = preprocess(img)  # Apply CONCH preprocessing
        ...
        # Extract features
        outputs = model.encode_image(images, proj_contrast=False, normalize=False)
...
# Save Results
pd.DataFrame(results).T.to_csv("conch_result.csv")