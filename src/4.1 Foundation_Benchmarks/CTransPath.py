...
class CTransPathEncoder(nn.Module):
    def __init__(self, model_path):
        super(CTransPathEncoder, self).__init__()
        ...
        # Initialize BYOL model with ResNet50 as the backbone
        self.encoder = BYOL(
            net=base_encoder,
            image_size=256,
        ...
        # If the checkpoint is a dictionary, extract "model" key
        if "model" in checkpoint:
            state_dict = checkpoint["model"]
        else:
            state_dict = checkpoint  # If it's already the correct format

        # Use DataParallel for multi-GPU
        self.encoder = nn.DataParallel(self.encoder).cuda()

        # Load the state dictionary
        self.encoder.load_state_dict(state_dict, strict=False)  # Set strict=False to ignore extra keys

        # Remove the classification head
        self.encoder.module.online_encoder.net.head = nn.Identity()
    ...
    def forward(self, x):
        with torch.no_grad():
            _, embedding = self.encoder(x.cuda(), return_embedding=True)
...
# Save Results
pd.DataFrame(results).T.to_csv("ctranspath_results.csv")
